...
async function prepareIosOpts (opts) {
  ...
  opts.platformVersion = opts.platformVersion || await xcode.getMaxIOSSDK(opts);
...
async function getAndCheckXcodeVersion (caps) {
  let version;
  try {
    version = await xcode.getVersion(true,caps);
  } catch (err) {
    logger.debug(err);
...
async function getAndCheckIosSdkVersion (opts=null) {
  let versionNumber;
  try {
    versionNumber = await xcode.getMaxIOSSDK(opts);
  } catch (err) {
