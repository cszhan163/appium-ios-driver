...
import { runSimulatorReset, isolateSimulatorDevice, checkSimulatorAvailable,runSimulatorReinstallApp,
...
class IosDriver extends BaseDriver {
  ...
  async createSession (...args) {
    ...
    //logger.debug("type of : "+this.opts+"content: "+JSON.stringify(this.opts)+" caps: "+JSON.stringify(this.opts));
    ...
    if(this.caps.reinstall == 'true'){
      logger.debug('need reinstall app');
      this.keepAppToRetainPrefs = false;
    } else {
      logger.debug('no need reinstall app');
  ...
  async startSimulator () {
    ...
    //logger.debug("type of :+  "+this.opts+"content: "+JSON.stringify(this.opts));
    ...
    this.iosSdkVersion = await utils.getAndCheckIosSdkVersion(this.opts);
    logger.debug(`iOS SDK Version set to ${this.iosSdkVersion}`);

    if(this.opts.xcodepath){

      this.opts.instrumentsPath = await instrumentsUtils.getInstrumentsPath(this.opts);
      logger.debug("instrumentents path: "+ this.opts.instrumentsPath)
    }

    let timeout = _.isObject(this.opts.launchTimeout) ? this.opts.launchTimeout.global : this.opts.launchTimeout;
    let availableDevices = await retry(3, instrumentsUtils.getAvailableDevices, timeout,this.opts);
    ...
    let isInstalled = await this.sim.isAppInstalled(this.opts.bundleId);
    if(!isInstalled){
      logger.debug('reinstall the app '+this.opts.bundleId);
    }
    if(this.keepAppToRetainPrefs == false || !isInstalled){
      logger.debug('reinstall the app '+this.opts.bundleId + ' '+this.opts.app+ 'to sim '+this.sim.udid);
      await runSimulatorReinstallApp(this.sim,this.opts);
    }
  ...
  async startInstruments () {
    ...
    await B.all([
      ...
      this.instruments.launch(this.opts)
    ]);
  }
