...
const XCODE_CRUN   = "/usr/bin/xcrun"

const rootDir = path.resolve(__dirname, '../..');
const INST_STALL_TIMEOUT = 12000;

async function getInstrumentsPath (opts=null) {
  let instrumentsPath;
  try {
    
    //let {stdout} = await exec('xcrun', ['-find', 'instruments']);
    //instrumentsPath = (stdout || '').trim().replace('\n$', '');
    log.debug('get instrumentsPath opts: '+opts)
    instrumentsPath = await xcode.getInstrumentsPath(opts);
...
async function getAvailableDevices (timeout = INST_STALL_TIMEOUT, opts = null) {
  log.debug('Getting list of devices instruments supports');
  let instrumentsPath = await getInstrumentsPath(opts);
  let args = {timeout};
  let lines;
  try {
    let {stdout} = await exec(instrumentsPath, ['-s', 'devices'], args);
    lines = stdout.split('\n');
  } catch (err) {
